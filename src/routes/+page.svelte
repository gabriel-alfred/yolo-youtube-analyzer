<script lang="ts">
  ...
  function setNewTarget() {
    ...
    const distance = Math.sqrt(
      (targetX - 30) * (targetX - 30) + targetY * targetY,
    );
...
<div
  class="w-full min-h-screen bg-transparent flex flex-col items-center pt-12 px-16 gap-8 relative overflow-hidden"
>
  <!-- Título y descripción -->
  <div class="text-center mb-8">
    <h1 class="text-4xl font-bold text-orange-500 mb-3">
      {translations.dashboard.heroTitle}
    </h1>
    <p class="text-slate-300 text-lg max-w-2xl">
      {translations.dashboard.heroDescription}
    </p>
  </div>

  <!-- Contenedor principal centrado con diseño responsivo -->
  <div
    class="flex flex-col lg:flex-row items-center justify-center gap-16 w-full max-w-6xl"
  >
    <!-- Lado izquierdo: Eye -->
      ...
      >
        <defs>
          ...
          <clipPath id="eyeShape">
            <path
              d="M 10,100 Q 35,88 55,82 Q 75,78 105,78 Q 135,78 165,84 Q 185,90 200,100 Q 185,110 165,116 Q 135,122 105,122 Q 75,122 55,118 Q 35,112 10,100 Z"
            />
          </clipPath>
        </defs>
          ...
          <g class="eye-part">
            ...
            {#each Array(6) as _, ring}
              ...
              {#each Array(20) as _, i}
                {@const angle =
                  (i / 20) * Math.PI * 2 + scanPhase * 0.2 + ring * 0.3}
                {@const x =
                  150 + pupilX * 0.4 + Math.cos(angle) * ringRadius * 0.8}
            ...
            <circle
              ...
              opacity="0.5"
              transform="rotate({scanPhase * 30} {150 + pupilX * 0.4} {100 +
                pupilY})"
            />
    ...
    <div class="w-full lg:w-auto">
      <Card variant="glass" padding="md" hoverable={false}>
        <div class="space-y-4">
          <div
            class="relative rounded-lg overflow-hidden border border-orange-500/30 shadow-lg shadow-orange-500/20"
          >
            <video
              class="w-full h-auto"
              src="/video.mp4"
  ...
  <div class="w-full max-w-7xl mt-16 mb-12">
    <div class="text-center mb-8">
      <h2 class="text-3xl font-bold text-orange-500 mb-2">
        {translations.dashboard.dashboardTitle}
      </h2>
      <p class="text-slate-400">
        {translations.dashboard.dashboardSubtitle}
      </p>
    </div>
