<script lang="ts">
    ...
    let classStats = $derived.by(() => {
        ...
        analysis.detections.forEach((d) => {
            const current = stats.get(d.class) || {
                count: 0,
                avgConfidence: 0,
            };
            stats.set(d.class, {
                ...
                avgConfidence:
                    (current.avgConfidence * current.count + d.confidence) /
                    (current.count + 1),
            });
        });
...
{:else}
    ...
    <div class="mb-8">
        ...
        >
            <div class="flex-1">
                <h1 class="text-4xl font-bold text-red-100 mb-3">
                    {analysis.title}
                </h1>
                <div
                    class="flex flex-wrap items-center gap-4 text-sm text-red-300/70"
                >
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <Card variant="elevated" padding="none">
            <div
                class="aspect-video bg-slate-950 relative overflow-hidden group"
            >
                {#if analysis.resultPath}
                    <!-- svelte-ignore a11y-media-has-caption -->
                    <video
                        class="w-full h-full object-contain"
                        controls
                        src={convertFileSrc(analysis.resultPath)}
                    >
                    </video>
                {:else}
                    <iframe
                        class="w-full h-full"
                        src="https://www.youtube.com/embed/{analysis.id}"
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card variant="gradient" padding="lg">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-xl bg-orange-500/20 flex items-center justify-center flex-shrink-0"
                    >
                        <svg
        ...
        <div class="flex gap-2 border-b border-red-500/20">
            <button
                ...
                class="px-6 py-3 text-sm font-medium transition-all relative {selectedTab ===
                'overview'
                    ? 'text-red-100'
        ...
        {#if selectedTab === "overview"}
            <div class="grid lg:grid-cols-2 gap-6">
                ...
                <Card variant="default" padding="lg">
                    {#snippet header()}
                        <h3
                            class="text-lg font-bold text-red-100 flex items-center gap-2"
                        >
                            <svg
                    ...
                    <div class="space-y-4">
                        ...
                        >
                            ...
                            >
                            <span class="font-semibold text-red-100"
                                >{(analysis.minConfidence * 100).toFixed(
                                    0,
                                )}%</span
                            >
                        </div>
                ...
                <Card variant="default" padding="lg">
                    ...
                    >
                        {#each classStats as stat}
                            <div
                                class="bg-slate-800/40 rounded-lg p-3 border border-red-500/10"
                            >
                                <div
                                ...
                                <div class="flex items-center gap-2">
                                    <div
                                        class="flex-1 h-2 bg-slate-900/60 rounded-full overflow-hidden"
                                    >
                                        <div
                                            class="h-full bg-gradient-to-r from-red-500 to-orange-500 transition-all"
                                            style="width: {(
                                                stat.avgConfidence * 100
                                            ).toFixed(0)}%"
                                        ></div>
                                    </div>
                                    <span
        ...
        {:else if selectedTab === "detections"}
            <Card variant="default" padding="lg">
                    ...
                    >
                        ...
                        {#each analysis.detectedClasses as className}
                            <option value={className}>
                                {className} ({analysis.detections.filter(
                                    (d) => d.class === className,
                                ).length})
                            </option>
                        {/each}
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead
                            class="bg-slate-900/50 border-y border-red-500/20"
                        >
                            <tr>
                                <th
                        ...
                        <tbody class="divide-y divide-red-500/10">
                            {#each filteredDetections as detection}
                                ...
                                >
                                    <td class="px-4 py-3 text-sm text-red-100"
                                        >#{detection.frameNumber}</td
                                    >
                                    <td class="px-4 py-3 text-sm text-red-200"
                                        >{detection.timestamp}</td
                                    >
                                    <td class="px-4 py-3">
                                        <span
                                            class="px-2 py-1 bg-slate-800/60 border border-red-500/20 rounded text-xs text-red-200"
                                        >
                                            {detection.class}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span
                                            class="text-sm font-medium {getConfidenceColor(
                                    ...
                                    <td
                                        class="px-4 py-3 text-xs text-red-300/70 font-mono"
                                    >
                                        x:{detection.bbox.x} y:{detection.bbox
                                            .y} w:{detection.bbox.width} h:{detection
                                            .bbox.height}
                                    </td>
                                </tr>
                            {/each}
