    ...
    <div class="bg-slate-900/95 border border-red-500/30 rounded-xl shadow-2xl shadow-red-500/20 max-w-md w-full animate-in zoom-in-95 duration-200 {className}">
      ...
      <div class="flex items-start gap-4 p-6 pb-4">
        ...
        <div class="flex-shrink-0">
          <div class="w-12 h-12 rounded-xl {config.iconBg} flex items-center justify-center relative" aria-hidden="true">
            <!-- Glow effect for danger variant -->
            {#if variant === 'danger'}
              <div class="absolute inset-0 bg-red-500/30 rounded-xl blur-xl animate-pulse"></div>
            {/if}
            {#if icon}
            ...
            {:else}
              <svg class="w-6 h-6 {config.iconColor} relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Title and Message -->
        <div class="flex-1">
          <h2 id="confirmation-title" class="text-xl font-bold text-red-100 mb-2">
            {title}
          </h2>
          <p id="confirmation-description" class="text-sm text-red-100/70 leading-relaxed">
            {message}
          </p>
        </div>
      </div>
